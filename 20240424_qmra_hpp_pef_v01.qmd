...
One for apples is @frankish_farm_2024 and another one for assessing the initial contamination in apple cider is @duffy_monte_2002 and the potential of growth in @duffy_modeling_2001. One QMRA for apple juice and UV process conditions is @gayan_selection_2014.
...
inactivation_pef <- LogLinInactivation$new("Treatment_pef")$
  map_input("logN0", logN0)$ #We map logN0 to the initial count defined before
  map_input("D", Constant$new("D_pef", 50.4))$
...
inactivation_pef <- LogLinInactivation$new("Treatment_pef")$
  map_input("logN0", logN0)$ #We map logN0 to the initial count defined before
  map_input("D", Constant$new("D_pef", 50.4))$
  map_input("t", Constant$new("energy_mu", 120))
```
