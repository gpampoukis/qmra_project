...
# Extract min and max values from logN0 to perform arithmetic
logN0_min_value <- logN0$inputs$min$value
logN0_max_value <- logN0$inputs$max$value

# Define the intercept constant
intercept_pef <- 0.72907

# Adjust the min and max values of logN0 by subtracting the intercept
logN0_pef_min_value <- logN0_min_value - intercept_pef
logN0_pef_max_value <- logN0_max_value - intercept_pef

# Create the new Uniform distribution for logN0_pef
logN0_pef <- Uniform$new("logN0_pef", level = 0)$
  map_input("min", Constant$new("logN0_pef_min", logN0_pef_min_value))$
  map_input("max", Constant$new("logN0_pef_max", logN0_pef_max_value))
...
# Define the initial logN0 constant
logN0 <- Constant$new("logN0", 1)

# Extract the value from logN0 to perform arithmetic
logN0_value <- logN0$value

# Define the intercept constant
intercept_pef <- 0.72907

# Calculate the adjusted logN0_pef value
logN0_pef_value <- logN0_value - intercept_pef

# Create the new Constant object for logN0_pef
logN0_pef <- Constant$new("logN0_pef", logN0_pef_value)
...
stor_time <- Constant$new("Storage time", 36.5)
...
abuse_storage$point_estimate()
stor_time$point_estimate()
change_storage_ideal$point_estimate()
change_storage_abuse$point_estimate()
tp_ideal_storage$point_estimate()
hpp_ideal_storage$point_estimate()
pef_ideal_storage$point_estimate()
tp_abuse_storage$point_estimate()
hpp_abuse_storage$point_estimate()
pef_abuse_storage$point_estimate()
consumer_dose_tp$point_estimate()
consumer_dose_hpp$point_estimate()
consumer_dose_pef$point_estimate()
