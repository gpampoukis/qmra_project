...
create_pill <- function(name, dose_input) {
  # Define the weighted average r value
  r_children <- 9.3e-3
  r_adults <- 5.1e-3
  population_children <- 0.2
  population_adults <- 0.8
  r_value <- (r_children * population_children) + (r_adults * population_adults)
  DoseResponse_Exponential$new(name)$  # Create a new DoseResponse_Exponential object with the given name
    map_input("r", Constant$new("r_dr", r_value))$  # Map the 'r' input to the weighted average r value
    map_input("dose", dose_input)  # Map the 'dose' input to the provided 'dose_input' argument
}
